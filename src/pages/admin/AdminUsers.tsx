...
import { Loader2, Search, Edit, Mail, Phone, Trash2, MessageCircle } from 'lucide-react';
    ...
    <div className="space-y-6">
                  ...
                  <TableRow key={u.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <TableCell>
                      <Badge className={cn(
                        ...
                        u.kyc_status === 'pending' ? "bg-orange-100 text-orange-700" : "bg-slate-100 text-slate-500"
                      )}>
                        {u.kyc_status === 'verified' ? 'Aprovado' : u.kyc_status === 'pending' ? 'Em Análise' : 'Pendente'}
                      </Badge>
                    </TableCell>
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="max-w-3xl rounded-3xl">
          ...
          {selectedUser && <UserManager user={selectedUser} onSuccess={() => { setIsEditOpen(false); fetchUsers(); }} />}
        </DialogContent>
      </Dialog>
    </div>
  );
