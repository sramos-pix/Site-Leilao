  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const { error: updateError } = await supabase
        .from('profiles')
        .update({ kyc_status: 'pending', document_url: publicUrl })
        .eq('id', user.id);
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-2xl">
        <Button variant="ghost" onClick={() => navigate('/app')} className="mb-6 text-slate-500 hover:bg-white rounded-xl">
        ...
        <Card className="border-none shadow-sm rounded-3xl overflow-hidden">
          <CardHeader className="bg-slate-900 text-white p-8">
            ...
            <p className="text-slate-400 text-sm mt-2">Seu documento será armazenado de forma segura para validação.</p>
          </CardHeader>
          <CardContent className="p-8">
            <div className="space-y-6">
              <div className="border-2 border-dashed border-slate-200 rounded-2xl p-8 text-center hover:border-orange-500 transition-colors cursor-pointer relative min-h-[240px] flex flex-col items-center justify-center bg-slate-50/50">
                <input type="file" className="absolute inset-0 opacity-0 cursor-pointer z-10" onChange={handleFileChange} accept="image/*,.pdf" disabled={isUploading} />
                {preview ? (
                  ...
                  <>
                    <div className="bg-white p-4 rounded-full shadow-sm mb-4"><Upload className="text-orange-500" size={32} /></div>
                    <p className="text-slate-700 font-semibold">Clique para selecionar seu RG ou CNH</p>
                  </>
                )}
              </div>
              <Button onClick={handleUpload} disabled={!file || isUploading} className="w-full h-14 bg-orange-500 hover:bg-orange-600 text-white rounded-xl font-bold text-lg shadow-lg shadow-orange-100">
